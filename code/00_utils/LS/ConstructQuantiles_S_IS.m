function M = ConstructQuantiles_S_IS(targets,factors,tau,h,H)

    %   CONSTRUCTQUANTILES_S_IS Construct in-sample scale-factor quantile forecasts.
    ...
    %   and the first-horizon lag of the target variable. The scale equation
    %   includes an intercept and the selected factor series, so factors enter
    %   the model only through the scale equation.
    %
    %   Rows with missing scale regressors are excluded, and the model is
    %   estimated using lsmem_garch.
    %
    ...
    for i = 1:N 
        ...
        X_scale = [ ones(numel(y),1) , factors(1:t-1,t) ];
        ...
        valid_rows = ~any(isnan(X_scale),2);
        y = y(valid_rows);
