...
%% Run QR models for all factor sets
...
for n_fac = {'singletons', 'pairs', 'triplets'}
    ...
    for h = 1:numel(H)
        ...
        for k = 1:K
            futures(k) = parfeval(@ConstructQuantiles_QR, 1, t_0, targets, squeeze(factors(:,k,:)), tau, h, H);
        end
    ...
    save(sprintf('../../data/outputs/quantiles/QR_OOS_%s.mat',n_fac),'M','factors_names')
end