...
# Load data
data <- read_dta('firmquarter_2022q1.dta')
data <- setDT(data)
...
# save the data
fwrite(agg_data_month[,.(date,PRisk_avg)], "../factors_clean/PRisk.csv", col.names = FALSE)
fwrite(agg_data_month[,.(date,NPRisk_avg)], "../factors_clean/NPRisk.csv", col.names = FALSE)
fwrite(agg_data_month[,.(date,Risk_avg)], "../factors_clean/Risk.csv", col.names = FALSE)